        ...
        .then(content => {
            if (convertToHtml) {
                ...
                for (var i = 0; i < newBody.childNodes.length; i++) {
                    ...
                    if (node.attributes)
                        for (var j = 0; j < node.attribuftes.length; j++) {
...
function showDialog(dataSource, title = "", options = {}) {
    ...
    document.querySelector('#dvContent').appendChild(dialog);
    dialogs.push(dialog);
...
//-----------
function callService(name, data = {}, background = false) {
    return new Promise(async (resolve, reject) => {
        console.log("ok");
        try {
            data.Ticket = localStorage.getItem("ticket");

            var values = background ? await API.callMethodsInBackground([name], [data]) : await API.callMethods([name], [data])
            var result = values[0];
            ...
            if (e.status)
                if (e.status == 400) {
                    ...
                    return reject(e.statusText);
                }
            else if (!(e.name && e.name == "AbortError")) {
                return reject(e.message || e);
            }
        }
    });
}
function callServices(names, data = [{}], background = false) {
    return new Promise(async (resolve, reject) => {
        try {
            data = data.map(x => { x.Ticket = localStorage.getItem("ticket"); return x; });
            var values = background ? await API.callMethodsInBackground(names, data) : await API.callMethods(names, data)
            var result = values[0];
            if ("IsAuthenticated" in result && !result.IsAuthenticated)
                signOut();
